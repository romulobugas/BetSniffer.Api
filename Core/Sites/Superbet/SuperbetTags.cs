...
namespace BetSniffer.Api.Core.Sites.Superbet
{
    public static class SuperbetTags
    {
        ...
        {
            { 6, new List<string> { "Total de Gols", "Total de Gols do Time", "Total de Gols da Equipe" } },            
            { 1, new List<string> { "Total de Escanteios", "Total de Escanteios do Time" } },
            { 34, new List<string> { "Total de Cartões", "Total de Cartões do Time","Total de Cartões da Equipe" } },
            ...
            { 46, new List<string> { "1º Tempo - Total de Cartões", "1º tempo - Total de Cartões do Time", "1º Tempo - Total de Cartões da Equipe" } },
        ...
        public static string CaptureElementCode(IWebElement element)
        {
            ...
            var segments = classAttribute.Split('-', StringSplitOptions.RemoveEmptyEntries);
            return segments.LastOrDefault() ?? string.Empty; // Supondo que o código seja o último segmento da classe
        }
        ...
        public static void AddDynamicTags(string homeTeam, string awayTeam)
        {
            ...
            {
                { 20, new List<string> { $"Total de Gols do Time {homeTeam}", $"Total de Gols da Equipe {homeTeam}" } },
                { 21, new List<string> { $"Total de Gols do Time {awayTeam}", $"Total de Gols da Equipe {awayTeam}" } },
                { 8, new List<string> { $"Total de Escanteios do Time {homeTeam}" } },
                { 9, new List<string> { $"Total de Escanteios do Time {awayTeam}" } },
                { 35, new List<string> { $"Total de Cartões do Time {homeTeam}", $"Total de Cartões da Equipe {homeTeam}" } },
                { 36, new List<string> { $"Total de Cartões do Time {awayTeam}", $"Total de Cartões da Equipe {awayTeam}" } },
                ...
                { 47, new List<string> { $"1º tempo - Total de Cartões do Time {homeTeam}", $"1º tempo - Total de Cartões da Equipe {homeTeam}" } },
                { 48, new List<string> { $"1º tempo - Total de Cartões do Time {awayTeam}", $"1º tempo - Total de Cartões da Equipe {awayTeam}" } },
